/**
 * Shared error handler for Mercury API routes
...
/**
 * Handle Mercury API errors and return appropriate NextResponse
 */
export function handleMercuryError(
  error: unknown,
  operation: string
): NextResponse<MercuryErrorResponse> {
  const err = error as Error;

  // Log the error (but never the token)
  console.error(`Mercury ${operation} error:`, err.message);
