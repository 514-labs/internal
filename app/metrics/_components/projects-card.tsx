...
export function ProjectsCard({ startDate, endDate }: ProjectsCardProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["projects-metrics", startDate, endDate],
    queryFn: async () => {
      ...
      const response = await fetch(
        `/api/analytics/posthog/metrics/projects?${params}`
      );
      if (!response.ok) {
        throw new Error("Failed to fetch projects metrics");
      }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Cumulative Projects</CardTitle>
        <CardDescription>
          Projects created over time by organization
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <MetricCard
            ...
            value={totalProjects}
            description="All-time cumulative projects"
            trend="up"
          />
          <MetricCard
            title="Organizations"
          ...
          <MetricCard
            ...
            value={
              totalOrganizations > 0
                ? Math.round(totalProjects / totalOrganizations)
                : 0
            }
            description="Projects per organization"
            trend="neutral"
          />
        </div>
