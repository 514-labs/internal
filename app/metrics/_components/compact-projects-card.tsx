...
interface CompactProjectsCardProps {
  startDate: string;
  endDate: string;
}

export function CompactProjectsCard({
  startDate,
  endDate,
}: CompactProjectsCardProps) {
  const { data, isLoading, error } = useQuery({
    queryKey: ["projects-metrics-compact", startDate, endDate],
    queryFn: async () => {
  ...
  const metrics = data || {};
  const { totalProjects = 0, breakdownSeries = [] } = metrics;
  ...
  const chartData = React.useMemo(() => {
    ...
    return activeSeries.dataPoints.map((point: any) => ({
      date: point.date,
      projects: point.cumulativeTotal,
    }));
