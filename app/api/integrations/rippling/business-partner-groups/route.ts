/**
 * Rippling Business Partner Groups API Endpoint
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await client.getBusinessPartnerGroups();

    return NextResponse.json({ data });
  } catch (error) {
    return handleRipplingError(error, "fetch business partner groups");
  }
}
