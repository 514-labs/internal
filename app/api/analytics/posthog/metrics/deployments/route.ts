/**
 * API route for cumulative deployments metrics
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Fetch cumulative deployments metrics
    const metrics = await getCumulativeDeployments(
      {
        startDate,
        endDate,
      },
    ...
    console.error("Error fetching cumulative deployments metrics:", error);
    ...
    return NextResponse.json(
      { error: "Failed to fetch cumulative deployments metrics" },
      { status: 500 }
    );
  }
}
